...
ゲームで購入した繁殖牝馬など、データにある実在の繁殖牝馬を持っている場合は、「データの繁殖牝馬から追加」で検索してチェックを付け、まとめて追加します。
名前・区分（繁殖牝馬）・性別・毛色・血統・系統・能力はデータの値を使い、編集はできません。父母の登録もいりません。
記録できるのは写真とメモだけで、詳細のタブは出ません。
能力や血統表は、右上の「データを確認」で確認します。
追加した繁殖牝馬は、探索の起点の既定や、自家製種牡馬づくり・今年の種付けの対象になります。
