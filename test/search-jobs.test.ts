...
import { SearchJobQueue, type SearchJob } from '../server/search-jobs';
...
describe('探索ジョブ', () => {
  ...
  it.each([null, request.stallionPool[0]])('途中の種牡馬指定 %s を含め、画面の探索と同じ結果を残す', async (intermediateStallion) => {
    const req = { ...request, intermediateStallion };
    queue = new SearchJobQueue(db, () => toRecords(emptyUserData()), 2);
    const job = await until(queue, queue.enqueue('lineage', req).id, (j) => j.status === 'done' || j.status === 'failed');
  ...
  it('待機中の中止は即座に、探索中の中止は区切りで止まり、それまでの結果を残す。同時実行は上限まで', async () => {
    queue = new SearchJobQueue(db, () => toRecords(emptyUserData()), 1);
