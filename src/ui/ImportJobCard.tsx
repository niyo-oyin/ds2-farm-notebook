...
import { applyMasterFields, compareAncestorFactors, homebredBlockReason, masterDiffRows, masterFromReading, type MasterFieldKey } from '../core/master-edits';
...
function PedigreeResult({ job, reading, onDone }: { job: ImportJob; reading: PedigreeScreen; onDone: (info: AppliedInfo) => void }) {
  ...
  const factorRows = useMemo(() => compareAncestorFactors(p.ancestors ?? [], app.ctx.ancestors), [p.ancestors, app.ctx.ancestors]);
  const [factorSkip, setFactorSkip] = useState<Set<string>>(() => new Set());
  const factorTargets = factorRows.filter((r) => r.status !== '一致' && !factorSkip.has(r.name));
  const saveFactors = () => { if (factorTargets.length) store.saveAncestorEdits(factorTargets.map((r) => ({ name: r.name, system: app.ctx.ancestors.get(r.name)?.system ?? null, sex: app.ctx.ancestors.get(r.name)?.sex ?? null, effects: r.factors }))); };
  const apply = () => {
    if (!target) {
  ...
  return <div className="import-card">
    ...
    {factorRows.length > 0 && <details className="import-factors" open={factorRows.some((r) => r.status !== '一致')}>
      ...
      <table className="import-diff"><tbody>{factorRows.map((r) => <tr key={r.name} className={r.status === '一致' ? 'muted' : factorSkip.has(r.name) ? 'excluded' : ''}>
        <td>{r.status !== '一致' && <input type="checkbox" aria-label={`${r.name} の因子を反映する`} checked={!factorSkip.has(r.name)} onChange={(e) => setFactorSkip((s) => { const n = new Set(s); if (e.target.checked) n.delete(r.name); else n.add(r.name); return n; })} />}</td>
        <th>{r.name}</th><td className="muted">{r.known ? (r.known.length ? r.known.join('・') : '因子なし') : '未登録'}</td><td>→</td><td>{r.factors.length ? r.factors.join('・') : '因子なし'}<span className="small muted"> {r.status}</span></td></tr>)}</tbody></table>
    </details>}
    <div className="import-apply">
...
function MasterResult({ reading, onDone }: { reading: MasterScreen; onDone: (info: AppliedInfo) => void }) {
  ...
  const existing = list.find((h) => norm(h.name) === norm(m.name));
  const next = useMemo(() => masterFromReading(kind, m, existing ?? null, app.master, app.ctx.ancestors), [kind, m, existing, app]);
  const rows = useMemo(() => masterDiffRows(existing ?? null, next, app.master.meta.bigSystems), [existing, next, app.master.meta.bigSystems]);
  ...
  const [err, setErr] = useState('');
  const apply = () => {
    try { onDone(existing ? applyMasterJob(existing, applyMasterFields(existing, next, selected)) : addMasterJob(next)); }
  ...
  return <div className="import-card">
    ...
    {block ? <div className="error">{block}</div> : <>
      ...
      <div className="import-apply"><button type="button" className="primary" disabled={!!existing && rows.length > 0 && selected.length === 0} onClick={apply}>{existing ? `${existing.name} を更新${rows.length ? `（${selected.length}項目）` : ''}` : `${label}として追加`}</button>{err && <span className="error">{err}</span>}</div>
    </>}
  </div>;
}
